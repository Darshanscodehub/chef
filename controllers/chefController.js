...
exports.createOrUpdateProfile = async (req, res) => {
    ...
    if (specialties) {
        profileFields.specialties = typeof specialties === 'string' 
...
exports.getVerifiedChefs = async (req, res) => {
    try {
        // You can add logic here to filter by 'isVerified: true' if you add that field later
        const profiles = await ChefProfile.find().populate('user', ['name', 'email']);
        res.json(profiles);
    } catch (err) {
        console.error(err.message);
...
exports.addDish = async (req, res) => {
    try {
        ...
        // Add to beginning of menu array
        profile.menu.unshift(newDish);
        await profile.save();

        res.json(profile.menu);
    } catch (err) {
        console.error(err.message);
        res.status(500).send('Server Error');
    }
